...
function roleAdmin(req: Request, res: Response, next: NextFunction): void {
    ...
    if (status_role != 'Admin' && status_role != 'admin') { // ถ้า role ไม่ใช่ 'RootAdmin' หรือ 'Admin'
        const response = new ServiceResponse( // สร้างตัวแปร response และกำหนดค่าด้วย new ServiceResponse
            ...
            "Unauthorized admin",    // กำหนดข้อความ "Unauthorized" ให้กับ response.message
            null,
