import { TypeShelfExport } from "@/types/response/reponse.msproduct copy"
...
export const TabsExportContent = ({ exportData, exportTabs = false }: { exportData?: TypeShelfExport, exportTabs?: boolean }) => {
    ...
    function handleDocumentSelect(doc: any): void {
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="bg-gray-50">
                            ...
                            {!exportTabs && <th className="p-3 text-left font-semibold text-gray-900">การจัดการ</th>}
                        </tr>
                    </thead>
                    <tbody>
                        {shelfBoxStorage.map((doc) => (
                            <tr key={doc.document_product_no} className="border-t hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td className="p-3 flex gap-2">
                                    ...
                                    {!exportTabs && <ExportButtonDialog storage_id={doc.storage_id} wareHouse={exportData.warehouse.master_warehouse_id} zone={exportData.zone.master_zone_id} />}
                                </td>
                            </tr>
                        ))}
                    </tbody>
