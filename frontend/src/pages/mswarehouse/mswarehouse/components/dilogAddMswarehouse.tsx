...
const DialogAdd = ({ getMswarehouseData }: DialogMswarehouseProps) => {
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger>
                <Button
                    className={`inline-flex items-center gap-2 px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg shadow-md transition-all duration-200 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                    <Plus size={18} />
                    Create Warehouse
                </Button>
