...
import { createMsrack, getMsrack } from "@/services/msrack.services";
...
const DialogAddRack = ({ zoneId, zoneName, zoneVolume, onRackAdded }: DialogAddRackProps) => {
    ...
    const [remainingSpace, setRemainingSpace] = useState<number>(zoneVolume);
    ...
    // Calculate remaining space in zone
    useEffect(() => {
    ...
    const handleSubmit = async (e?: React.MouseEvent) => {
        ...
        // Check if rack exceeds remaining zone space
        if (volume > remainingSpace) {
            setError(`Rack volume (${volume.toLocaleString()} cm³) exceeds remaining zone space (${remainingSpace.toLocaleString()} cm³)`);
            return;
        }
        ...
        try {
            const rackData = {
                ...
                description,
                master_zone_id: zoneId
            };

            const response = await createMsrack(rackData);
            ...
            setError("An error occurred while creating the rack");
            console.error(err);
        } finally {
    ...
    return (
        <Dialog.Root>
            ...
            <Dialog.Content className="bg-white rounded-xl shadow-xl p-6 max-w-md">
                <Dialog.Title className="text-xl font-bold mb-4">Add New Rack to {zoneName}</Dialog.Title>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                        <div className="text-sm font-medium text-blue-800 mb-1">Zone Space</div>
                        <div className="grid grid-cols-2 gap-2 text-sm">
                            <div>
                                <span className="text-gray-600">Total:</span> {zoneVolume.toLocaleString()} cm³
                            </div>
                            <div>
                            ...
                            <div>
                                <span className="text-gray-600">Usage:</span> {zoneVolume > 0 ? ((usedSpace / zoneVolume) * 100).toFixed(2) : 0}%
                            </div>
                        </div>
                    </div>
